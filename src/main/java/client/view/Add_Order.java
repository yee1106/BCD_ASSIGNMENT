...
public class Add_Order extends javax.swing.JFrame {
  ...
  private void configCreateBlock(){
    ...
    Block blk = new Block(lastBlockHeader.getIndex()+1, lastBlockHeader.getCurrentHash(), order.getOrder_batch_id(), order.getFrom(), order.getTo(), tranx, mt.getRoot());
    Blockchain.nextBlock( blk );
    Blockchain.distribute();
    clinic_healthcare.tableAddOrder(order.getOrder_batch_id(), order.getVaccineType(), order.getQuantity(), order.getTo(), blk);
  }
  
  public Order createOrder(){
    long new_batch_id = Blockchain.DB.getLast().getHeader().getBatch_id() + 1;
    Date today = new Date();
    Order order = new Order(new_batch_id, vaccineTypeComboBox.getSelectedItem().toString(), Integer.valueOf(quantityTextField.getText()), current_user.getUserName()
